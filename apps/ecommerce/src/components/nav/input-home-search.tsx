...
const InputHomeSearch = () => {
  ...
  return (
      ...
      >
          ...
          <div
              className={cn(
                  "fixed lg:absolute top-[53px] z-50 invisible shadow-lg left-0 right-0 mx-2 lg:mx-0 p-4 opacity-0 rounded-lg bg-white",
                  {
          ...
          {value?.length > 0 && (
              <X
                  onClick={() => setValue("")}
                  className="text-muted-foreground cursor-pointer absolute right-2 top-[50%] translate-y-[-50%] -z-40"
                  size={18}
              />
          )}
          <span
              onClick={toggleFocus}
              className={cn(
                  "fixed transition-opacity -inset-1 w-dvw top-16 opacity-0 invisible bg-black/50 z-30",
                  {
                      "opacity-1 visible": onFocus,
                  }
              )}
